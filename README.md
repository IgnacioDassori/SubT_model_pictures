...
Estas se deben correr cada vez que uno quiera iniciar un nuevo contenedor a excepción de la primera y tercera, que sólo serán necesarias la primera vez. Con esto tenemos nuestro container listo para empezar a simular con los mundos y modelos predeterminados, pero lo que queremos es lograr correr los mundos y modelos custom así que veremos como hacer eso a continuación.
...
El script necesario para correr el programa se encuentra dentro del repositorio que hemos bajado de gitlab. A continuación se muestra el código contenido en el script:
...
cp -RT ~/subt_ws/src/amtc-packages/room_and_pillar_circuit/models/ ~/.ignition/fuel/fuel.ignitionrobotics.org/openrobotics/models 
cp  ~/subt_ws/src/amtc-packages/room_and_pillar_circuit/cave_circuit_practice_01.sdf ~/subt_ws/src/subt/subt_ign/worlds/cave_circuit_practice_01.sdf
cd ~/subt_ws
catkin_make install


ign launch -v 4 cave_circuit.ign circuit:=cave   worldName:=cave_circuit_practice_01     robotName1:=X1    robotConfig1:=AMTC_LHD_SENSOR_CONFIG_1 localModel:=true enableGroundTruth:=true
